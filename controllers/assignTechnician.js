...
const assignTechnician = async (req, res) => {
  ...
  try {
    const updatedAppointment = await Appointment.findByIdAndUpdate(
      appointmentId,
      { tech: technicianId }, // Assign the technician
      { new: true } // Return the updated document
    ).populate("tech", "name userId");
    ...
    if (updatedAppointment.tech && updatedAppointment.tech.userId) {
      await createNotification(
        updatedAppointment.tech.userId,
        "technician",
        `You have been assigned to appointment for vehicle ${updatedAppointment.vehicleNumber} (${updatedAppointment.model})`,
        "new_appointment",
