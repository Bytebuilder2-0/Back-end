...
const updateWorkload = async (req, res) => {
	...
	try {
		...
		if (!Array.isArray(workload) || workload.length === 0) {
			return res.status(400).json({ message: "Workload must be a non-empty array" });
		}
		...
		for (const task of workload) {
			if (
				...
				(task.status && !["Pending", "In Progress", "Completed"].includes(task.status))
			) {
...
const upadateWorkloadStatus = async (req, res) => {
	try {
		...
		if (!status || !validStatuses.includes(status)) {
			return res.status(400).json({
				message: "Invalid status, valid values are: Pending, In Progress, Completed",
			});
		}
...
const getTechnicianAppointmentCount = async (req, res) => {
	try {
		...
		if (technicianAppointments.length === 0) {
			return res.status(404).json({ message: "No appointments found for technicians" });
		}

		res.status(200).json({ technicianAppointments });
	} catch (error) {
		console.error("Error fetching technician appointment counts:", error);
		res.status(500).json({ message: "Error fetching technician appointment counts" });
	}
};
// appointmentController.js
...
const getDepartmentStatusData = async (req, res) => {
	try {
		...
		const departments = [...new Set(appointmentData.map((item) => item.department))]; // Get unique departments
